...
@interface ZKImgRunLoopView () <UIScrollViewDelegate>
    ...
    @property (nonatomic,strong) YhbPageControl *pageControl;
    /** 当前图片索引*/
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (point.x > self.frame.size.width*(self.imgCount-2)+self.frame.size.width*0.5 && !self.timer) {
        self.pageControl.currentPage = 0;
    }else if (point.x > self.frame.size.width*(self.imgCount-2) && self.timer && isRight){
        self.pageControl.currentPage = 0;
    }else{
        self.pageControl.currentPage = (point.x + self.frame.size.width*0.5) / self.frame.size.width;
    }
...
- (void)nextPage
    {
        [self.scrollView setContentOffset:CGPointMake((self.pageControl.currentPage+1)*self.frame.size.width, 0) animated:YES];
    }
- (void)stopTimer
    {
...
- (YhbPageControl *)pageControl{
    if (!_pageControl) {
        CGRect rect = CGRectMake(0, self.frame.size.height-AUTO(15), self.frame.size.width, AUTO(2));
        _pageControl = [[YhbPageControl alloc] initWithFrame:rect];
        [_pageControl layoutSubviews];
        self.pageControl.currentPage = 0;

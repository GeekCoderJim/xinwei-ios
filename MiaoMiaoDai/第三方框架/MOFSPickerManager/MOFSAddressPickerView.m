...
@property (nonatomic, strong) NSMutableArray<MOFAddressModel *> *dataArr;
...
- (void)initToolBar {
    ...
    self.toolBar.backgroundColor = [YhbMethods colorWithHexString:COLOR_MAIN];
}
    ...
    self.toolBar.commitBlock = ^ {
        if (commitBlock) {
            ...
            if (weakSelf.dataArr.count > 0) {
               MOFAddressModel *addressModel = weakSelf.dataArr[weakSelf.selectedIndex_province];
                CityModel *cityModel;
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary<NSString *,NSString *> *)attributeDict {
    if ([elementName isEqualToString:@"province"]) {
        MOFAddressModel *model = [[MOFAddressModel alloc] initWithDictionary:attributeDict];
...
- (NSString *)searchByKey:(NSString *)key valueName:(NSString *)valueName type:(NSString *)type {
    ...
    MOFAddressModel *addressModel = (MOFAddressModel *)[self searchModelInArr:_dataArr key:arr[0] type:type];
    if (addressModel) {
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {
        MOFAddressModel *addressModel = self.dataArr[row];
        return addressModel.name;
    } else if (component == 1) {
        MOFAddressModel *addressModel = self.dataArr[self.selectedIndex_province];
...
//        [pickerLabel setBackgroundColor:[UIColor clearColor]];
        [pickerLabel setFont:[UIFont systemFontOfSize:AUTO(14)]];
    }
